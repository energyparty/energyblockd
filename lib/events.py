...
D = decimal.Decimal
COMPILE_MARKET_PAIR_INFO_PERIOD = 30 * 60
COMPILE_ASSET_MARKET_INFO_PERIOD = 30 * 60
...
def expire_stale_btc_open_order_records():
    ...
    if num_stale_records: logging.warn("REMOVED %i stale ENRG open order objects" % num_stale_records)
...
def generate_wallet_stats():
    ...
    gevent.spawn_later(60 * 60, generate_wallet_stats)
...
def compile_extended_feed_info():
    ...
    gevent.spawn_later(60 * 30, compile_extended_feed_info)
