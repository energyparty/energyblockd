...
from lib import config, util, blockchain, util_bitcoin
from lib.components import betting, assets, assets_trading, dex

PREFERENCES_MAX_LENGTH = 100000
API_MAX_LOG_SIZE = 10 * 1024 * 1024
API_MAX_LOG_COUNT = 10

decimal.setcontext(decimal.Context(prec=6, rounding=decimal.ROUND_HALF_EVEN))
D = decimal.Decimal
...
def serve_api(mongo_db, redis_client):
    ...
    def is_ready():
        ...
        try:
           ...
           return {
               ...
               'cw_last_message_seq': config.cw_last_message_seq,
               'block_height': blockchainInfo['info']['blocks'],
               'testnet': config.TESTNET,
               'ip': ip,
               'num_connections': blockchainInfo['info']['connections'],
               'country': country
           }
    ...
    def get_normalized_balances(addresses):
        ...
        for d in results:
            ...
            d['normalized_quantity'] = util_bitcoin.normalize_quantity(d['quantity'], divisible)
    ...
    def get_raw_transactions(address, start_ts=None, end_ts=None, limit=500):
        ...
        txns.sort(key=operator.itemgetter('block_index'), reverse=True)
        return txns
    ...
    def get_order_book_buysell(buy_asset, sell_asset, pct_fee_provided=None, pct_fee_required=None):
        ...
        if base_asset == config.BTC:
            ...
            if buy_asset == config.BTC:
                ...
                ask_book_min_pct_fee_required = pct_fee_required
            elif sell_asset == config.BTC:
                bid_book_min_pct_fee_provided = pct_fee_provided
    ...
    def get_users_pairs(addresses=[], max_pairs=12):
        return dex.get_users_pairs(addresses, max_pairs, hide_btc=True)
    ...
    def get_latest_wallet_messages(last_seq):
        ...
        results = list(mongo_db.wallet_messages.find({"_id": {"$gte": last_seq}}, sort=[("_id", pymongo.ASCENDING)]))
        logging.debug("get_latest_wallet_messages: last seq {}, returning {} entries".format(last_seq, len(results)))
        return results
    ...
    def handle_get():
        ...
        try:
            ...
            r = grequests.map((grequests.post(url, data=json.dumps(payload), headers={'content-type': 'application/json', 'Connection': 'close'}),))[0]
            if r is None:
            ...
            cbd_result = r.json()
            if 'error' in cbd_result:
                cbd_result_valid = False
                cbd_result_error_code = "GOT ERROR: %s" % cbd_result['error']
        cbd_e = time.time()
        ...
        result = {
            'energypartyd': 'OK' if cpd_result_valid else 'NOT OK',
            'energyblockd': 'OK' if cbd_result_valid else 'NOT OK',
            'energyblockd_error': cbd_result_error_code,
            'energypartyd_ver': '%s.%s.%s' % (
                cpd_status['version_major'], cpd_status['version_minor'], cpd_status['version_revision']) if cpd_result_valid else '?',
            'energyblockd_ver': config.VERSION,
            'energypartyd_last_block': cpd_status['last_block'] if cpd_result_valid else '?',
            'energypartyd_last_message_index': cpd_status['last_message_index'] if cpd_result_valid else '?',
            'energypartyd_check_elapsed': cpd_e - cpd_s,
            'energyblockd_check_elapsed': cbd_e - cbd_s,
        }
