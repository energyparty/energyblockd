...
def calculate_price(base_quantity, quote_quantity, base_divisibility, quote_divisibility, order_type=None):
    ...
    try:
        if order_type == 'BUY':
            decimal.setcontext(decimal.Context(prec=6, rounding=decimal.ROUND_DOWN))
        elif order_type == 'SELL':
            decimal.setcontext(decimal.Context(prec=6, rounding=decimal.ROUND_UP))

        price = format((quote_quantity) / (base_quantity), '.6f')
...
def get_xcp_and_btc_pairs(exclude_pairs=[], max_pairs=12, from_time=None):
    ...
    for currency in ['XEP', 'ENRG']:
        ...
        for currency_pair in currency_pairs:
            if currency_pair['pair'] == 'XEP/ENRG':
...
def get_users_pairs(addresses=[], max_pairs=12, hide_btc=True):

    top_pairs = []
    all_assets = []
    exclude_pairs = []
    quote_assets=['XEP'] if hide_btc else ['XEP', 'ENRG']
    ...
    for currency in quote_assets:
        if len(top_pairs) < max_pairs:
            ...
            for currency_pair in currency_pairs:
                ...
                if currency_pair['pair'] == 'XEP/ENRG': # XEP/ENRG always in first
                    top_pairs.insert(0, top_pair)
                else:
                    top_pairs.append(top_pair)
                all_assets += [currency_pair['base_asset'], currency_pair['quote_asset']]

    if not hide_btc and ('XEP/ENRG' not in [p['base_asset'] + '/' + p['quote_asset'] for p in top_pairs]):
        top_pairs.insert(0, {
            'base_asset': 'XEP',
            'quote_asset': 'ENRG'
        })
    ...
    if hide_btc:
       for p in top_pairs:
           if 'ENRG' in p['quote_asset']: top_pairs.remove(p)
    ...
    for p in range(len(top_pairs)):
        ...
        top_pairs[p]['price'] = format(price, ".6f")
        top_pairs[p]['trend'] = trend
        top_pairs[p]['progression'] = format(progression, ".2f")
        top_pairs[p]['price_24h'] = format(price24h, ".6f")
...
def get_assets_supply(assets=[]):
    ...
    if 'XEP' in assets:
        supplies['XEP'] = (util.call_jsonrpc_api('get_xcp_supply', [])['result'], True)
        assets.remove('XEP')

    if 'ENRG' in assets:
        supplies['ENRG'] = (0, True)
        assets.remove('ENRG')
...
def get_markets_list(mongo_db=None):
    ...
    pairs += get_xcp_and_btc_pairs(exclude_pairs=pair_with_volume, max_pairs=50 - len(pair_with_volume))
    ...
    for pair in pairs:
        ...
        market['price'] = format(price, ".6f")
        market['trend'] = trend
        market['progression'] = format(progression, ".2f")
        market['price_24h'] = format(price24h, ".6f")
        ...
        if market['base_asset'] == 'XEP' and market['quote_asset'] == 'ENRG':
