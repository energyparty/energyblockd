...
def get_price_primatives(start_dt=None, end_dt=None):
    ...
    xcp_btc_price = mps_xcp_btc['market_price'] if mps_xcp_btc else None
    btc_xcp_price = calc_inverse(mps_xcp_btc['market_price']) if mps_xcp_btc else None
...
def compile_summary_market_info(asset, mps_xcp_btc, xcp_btc_price, btc_xcp_price):
    ...
    return {
        'price_in_{}'.format(config.XCP.lower()): price_in_xcp,
        'price_in_{}'.format(config.BTC.lower()): price_in_btc,
        'price_as_{}'.format(config.XCP.lower()): calc_inverse(price_in_xcp) if price_in_xcp else None,
        'price_as_{}'.format(config.BTC.lower()): calc_inverse(price_in_btc) if price_in_btc else None,
...
def compile_7d_market_info(asset):
    ...
    _7d_history_in_xcp = None
    _7d_history_in_btc = None
    if asset not in [config.BTC, config.XCP]:
...
def compile_asset_pair_market_info():
    ...
    logging.info("Recomposed 24h trade statistics for %i asset pairs: %s" % (len(pair_data), ', '.join(list(pair_data.keys()))))
